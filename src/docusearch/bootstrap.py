...
def scan_repo(root: Path, *, max_files: int = 50000) -> RepoScan:
    ...
    Uses ``os.walk(followlinks=False)`` so a symlink cycle can never loop or raise, and prunes skip
    dirs before descending (so vendor trees like ``node_modules`` are never entered)."""
...
def _safe_name(name: str) -> str:
    """A source name is a plain label — strip anything that isn't identifier-safe so a crafted name
    can't inject YAML keys into the generated config (red-team #H3)."""
...
def bootstrap_config(root: Path, *, name: str | None = None) -> str:
    ...
    src_name = _safe_name(name or root.resolve().name or "source")
    globs = _globs_for(store_type, scan)
    # JSON-encode every interpolated string (a valid YAML scalar) so a hostile path / filename /
    # extension can never break out of its value or inject a new key (red-team #H3).
    inc = ", ".join(json.dumps(g) for g in globs)
    loc = json.dumps(root.as_posix())
    root_display = re.sub(r"\s+", " ", str(root))  # never let a newline in the path break the comment
    ...
    return (
        f"# Generated by `docusearch bootstrap` from {root_display} — review before use.\n"
