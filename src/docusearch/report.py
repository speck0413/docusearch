...
def render_report(
    *,
    ...
    meta = _meta(
        stamp, requested_by, model, embed_model, audience, sources, run_id,
        effort=effort, model_effort=model_effort,
    )
...
#: Research effort the requester asked for. Deliberately the SAME vocabulary as the model
#: harness's own effort levels, so one word means one thing everywhere.
...
def _meta(
    ...
    # Two independent dials, so a reader can tell "shallow research" from "shallow thinking".
    if effort:
        rows.append(("🎚", f"research effort: {effort}"))
    if model_effort:
        rows.append(("⚙️", f"model effort: {model_effort}"))
    rows += [
